...
.DESCRIPTION
    ...
    6. Logs all actions and results to C:\temp\Win32appsArm64status.log
    7. Provides detailed console output with color coding
...
try {
    ...
    if (-not (Get-Module -ListAvailable -Name $moduleName -ErrorAction SilentlyContinue)) {
       write-host "Module $moduleName not found. Installing..." -Color "Yellow"
        
        # Install the module
        Install-Module -Name $moduleName -Force -AllowClobber -Scope CurrentUser -ErrorAction Stop
        write-host "Successfully installed $moduleName module" -Color "Green"
    }
    
    # Import the module
    Import-Module $moduleName -MinimumVersion $moduleRequiredVersion -ErrorAction Stop
    } catch {
        write-host "Failed to install or import $moduleName module: $_" -Color "Red"
    exit 1
}
#endregion
...
try {
    Write-Log -Message "Retrieving all Win32 apps..." -Level "INFO" -Color "Cyan"
...
- Apps with ARM64 found: $($summary.ARM64Found)
- Apps without ARM64: $($summary.ARM64NotFound)
...
Write-Log -Message "Script completed, Log file created at '$logFile'" -Level 'INFO' -Color 'Cyan'

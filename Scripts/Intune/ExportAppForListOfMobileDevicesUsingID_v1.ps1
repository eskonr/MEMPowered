<#
The script connects to Microsoft Graph to fetch detected apps for a supplied list of mobile devices and exports their app inventory.
Scopes:
...
# Helper: get all detected apps for a device (handles pagination + basic retries)
function Get-DetectedAppsAllPages {
    param([Parameter(Mandatory)] [string]$DeviceId)

    $all = @()
    $uri = "https://graph.microsoft.com/beta/deviceManagement/managedDevices('$DeviceId')/detectedApps?$top=999"
    ...
    while ($uri) {
        try {
            ...
            if ($resp.'@odata.nextLink') { $uri = $resp.'@odata.nextLink' } else { $uri = $null }
            $attempt = 0  # reset on success
        } catch {
            Write-Host "Detected apps request failed (attempt $attempt/$maxAttempts) for device $DeviceId : $($_.Exception.Message)" -ForegroundColor Yellow
            if ($attempt -ge $maxAttempts) {
                Write-Host "Giving up after $maxAttempts attempts for device $DeviceId." -ForegroundColor Red
                break
            }
            Start-Sleep -Seconds (2 * $attempt)  # simple backoff
        }
    }
    return $all
}

# Function to collect device data with full app inventory
function Collect-DeviceData {
    ...
    foreach ($deviceId in $devices) {
        try {
            ...
            # Fetch all detected apps (handles pagination)
            $detectedApps = Get-DetectedAppsAllPages -DeviceId $deviceId

            if (-not $detectedApps -or $detectedApps.Count -eq 0) {
                Write-Host "No detected apps returned for device $deviceId. Last sync: $($device.LastSyncDateTime)" -ForegroundColor Yellow
                continue
            }
...
if ($results) {
    # De-duplicate by DeviceId + AppDisplayName (ignoring version)
    $results = $results |
        Sort-Object DeviceId, AppDisplayName |
        Select-Object DeviceId, AppDisplayName, UserPrincipalName -Unique
